...
the expense-inflation variant, is in note 07.
...
as a cross-check. Note 07 §7.7 shows a commercial model that reports three
...
> correct total is $24,341.10, not the $16,966.10 printed (note 11 §5.5).
...
> one column headed "per quater" holding a monthly figure (note 11 §5.5).
...
maximum purchase price down by $58,033. Note 10 §4.5 makes the same point on
...
> from the Week 2 grandmother exercise (note 02, §2.9) — this is its second
...
> gross-up is the algebraically consistent one and is what note 10 §4.1 uses
